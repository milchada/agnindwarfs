...
def p_BH(Lx, vstar, Mstar, beta, fedd_min,fedd_max, fduty, Mstar0):
    ...
    Phi = p_if_on*fduty + (1 - fduty) #i.e. fduty chance that AGN is on but below detection limit, 
                                      #(1-fduty) chance that it is off; either way, BH exists. 
...
#same as in other models, tested.
    focc = f_occ(Mstar, Mstar0)
    p = np.power(focc, I_n) * np.power(1 - focc, 1-I_n)#np.power(, weights[i])
    return np.log10(p)
...
def log_pdf(theta, Lx, Mstar, vstar, Llim, priors, weights):
    beta, log_fduty, logMstar0 = theta
    fedd = f_edd(Lx, vstar)
    Mstar0 = 10**logMstar0
    fedd_min = fedd.min()
    fedd_max = fedd.max()
...
def run_emcee(galcat, filename, nwalkers=100, nsteps=int(1e4), nburn=100, 
                priors=np.array([[.1,4],[-1,0],[6,10]]), right=True):
    ndim = len(priors)
    Lx = galcat['Lx']
    Mstar = galcat['Mstar'] 
    vstar = galcat['vstar'] 
